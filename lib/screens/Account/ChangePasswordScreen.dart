...
class _ChangePasswordScreenState extends State<ChangePasswordScreen> {
  ...
  void _getPassword() {
    ...
    db.getConnection().then((conn) {
      String sqlQuery = "SELECT PPL.Peoples_Password FROM Customers CSM,Peoples PPL WHERE PPL.Peoples_Id=.Peoples_Id AND CSM.Customers_Id='CSM1'";

      conn.query(sqlQuery).then((results) {
        print('${results}');
        for(var row in results){
          classAccountClientData a=new classAccountClientData();
          a.clientPassword=row[0];
          getPassword=row[0];
...
List   getWidgetJob(List<classAccountClientData> list){

  List _list =[];
  for(int i=0;i<list.length;i++){
    _list.add({

      'Account_Password':'${list[i].clientPassword}',
